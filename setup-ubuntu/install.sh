#!/usr/bin/env bash
# Ubuntu post-install setup — master orchestrator
#
# Bare-metal Ubuntu work laptop — no gaming module. For Ubuntu under WSL use
# ../setup-ubuntu-wsl instead: it drops the GUI apps, the terminal emulator, the
# fonts and Docker Engine, none of which belong inside a WSL distro.
#
# Tooling is split in two:
#   system layer   apt — compilers, fish, git, terminal, fonts, Docker
...
Steps
  --all        Run every install step, in order:
               base → mise → langs → apps → dotfiles → claude → agents
...
Maintenance
  --update     Update everything already installed. Not part of --all:
               apt upgrade → mise up → rustup update → fisher update →
...
This is a work-laptop setup — no gaming module.
USAGE
}
...
log_step "Ubuntu post-install setup"
...
if [[ -n "${WSL_DISTRO_NAME:-}" ]] || grep -qi microsoft /proc/version 2>/dev/null; then
    log_warn "WSL detected — use ../setup-ubuntu-wsl instead; this variant installs GUI apps, fonts and Docker Engine"
fi
