...
if [[ $# -eq 0 ]]; then
    echo "Usage: $0 [--all] [--base] [--langs] [--apps] [--dotfiles] [--claude]"
    echo ""
    echo "  --all        Run all steps (base → langs → apps → dotfiles → claude)"
...
for arg in "$@"; do
    case "$arg" in
        --all)      RUN_BASE=true; RUN_LANGS=true; RUN_APPS=true; RUN_DOTFILES=true; RUN_CLAUDE=true ;;
        --base)     RUN_BASE=true ;;
...
if $RUN_BASE; then
    ...
    log_info "Shell: log out and back in for Fish to become your default shell"
fi
if $RUN_LANGS; then
