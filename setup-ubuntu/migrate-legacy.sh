#!/usr/bin/env bash
# Migrate an existing bare-metal Ubuntu environment from the pre-mise layout.
#
...
remove_pkg zoxide fzf bat ripgrep fd-find jq gh
...
# ── 10. Stale Fish universal PATH entries ──────────────────────────────────────
