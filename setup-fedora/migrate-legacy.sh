#!/usr/bin/env bash
# Migrate an existing bare-metal Fedora environment from the pre-mise layout.
#
...
remove_pkg zoxide fzf bat ripgrep fd-find jq chezmoi neovim starship lazygit gh

# ── 6b. COPRs that only existed for mise-managed tools ────────────────────────
# atim/starship and atim/lazygit were enabled purely to supply two binaries mise
# now owns. scottames/ghostty is NOT touched — Ghostty is still a dnf package.
log_step "COPR repositories superseded by mise"

for copr in atim/starship atim/lazygit; do
    if sudo dnf copr list --enabled 2>/dev/null | grep -q "${copr#*/}"; then
        (( PLANNED++ )) || true
        if $APPLY; then
            sudo dnf copr remove -y "$copr" 2>/dev/null || sudo dnf copr disable -y "$copr"
            log_success "Disabled COPR $copr"
        else
            log_info "WOULD DISABLE COPR $copr"
        fi
    else
        log_skip "COPR $copr (not enabled)"
    fi
done
