#!/usr/bin/env bash
# Migrate an existing CachyOS environment from the pre-mise layout.
#
...
remove_pkg() {
    ...
    if $APPLY; then
        ...
        # -Rns: also drop now-orphaned dependencies and their config
        paru -Rns --noconfirm "${present[@]}"
...
remove_pkg zoxide fzf eza bat ripgrep fd jq chezmoi neovim starship lazygit github-cli
...
# ── 8b. npm globals under the system Node prefix ──────────────────────────────
# The old packages/apps.sh ran `sudo npm install -g` against the *system* node,
# so Copilot CLI and Codex landed in /usr/lib/node_modules. Node now comes from
# mise, so those copies are unreachable and unmaintained — but they are owned by
# a package manager this script does not drive, so it reports rather than acts.
log_step "npm globals under the system Node prefix"

if [[ -d /usr/lib/node_modules/@github/copilot || -d /usr/lib/node_modules/@openai/codex ]]; then
    log_warn "Found npm globals installed against the system Node:"
    [[ -d /usr/lib/node_modules/@github/copilot ]] && log_info "  /usr/lib/node_modules/@github/copilot"
    [[ -d /usr/lib/node_modules/@openai/codex   ]] && log_info "  /usr/lib/node_modules/@openai/codex"
    log_info "  Codex is now installed by packages/agents.sh from its native installer,"
    log_info "  and Copilot CLI by packages/apps.sh against mise's Node. Remove the old"
    log_info "  copies by hand when you are happy with the replacements:"
    log_info "    sudo npm uninstall -g @github/copilot @openai/codex"
else
    log_skip "system-Node npm globals (none found)"
fi
