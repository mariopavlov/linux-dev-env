...
#   system layer   paru — compilers, fish, git, terminals, fonts, Docker
...
#
# On a rolling distro paru's versions are already current, so the mise split is
# not about freshness here — it is about this machine resolving the same tool
...
Steps
  --all        Run every install step, in order:
               base → mise → langs → apps → gaming → dotfiles → claude → agents
  --base       System layer: paru packages, Ghostty/Alacritty, fonts, Fish +
               Fisher, Docker, Git config
  ...
  --gaming     Steam, Lutris, Heroic, Wine/Proton
...
Maintenance
  --update     Update everything already installed. Not part of --all:
               paru -Syu → mise up → rustup update → fisher update →
...
for arg in "$@"; do
    case "$arg" in
        --all)      RUN_BASE=true; RUN_MISE=true; RUN_LANGS=true; RUN_APPS=true
                    RUN_GAMING=true; RUN_DOTFILES=true; RUN_CLAUDE=true; RUN_AGENTS=true ;;
        --base)     RUN_BASE=true ;;
        --mise)     RUN_MISE=true ;;
...
# This is the step the pre-mise setup was missing entirely — every guard was a
# bare `is_installed` check, so nothing outside paru was ever refreshed.
if $RUN_UPDATE; then
    log_step "Updating everything"

    log_step "System packages (paru)"
    paru -Syu --noconfirm
