...
# setup, no in-distro terminal emulator, no Docker Engine (use Docker Desktop's
# WSL integration instead — enable this distro under Settings > Resources > WSL
...
if [[ $# -eq 0 ]]; then
    echo "Usage: $0 [--all] [--base] [--langs] [--dotfiles]"
    echo ""
...
for arg in "$@"; do
    case "$arg" in
        --all)      RUN_BASE=true; RUN_LANGS=true; RUN_DOTFILES=true ;;
        --base)     RUN_BASE=true ;;
